...
if setup = false
	...
	for(var p = 0; p < page_number; p++)
		...
		//get the x position for the text box
			...
			//no character (centering the textbox on screen)
		...
		for (var c = 0; c < text_length[p]; c++)
			...
			//get current widthe of the line
...
if draw_char == text_length[page] && page == page_number -1
	...
	for (var op = 0; op < option_number; op++)
		...
		draw_sprite_ext(txtb_spr[page], txtb_img, _txtb_x + 16, _txtb_y - _op_space*option_number + _op_space*op - 8, _o_w/txtb_spr_w, (_op_space +2)/txtb_spr_h, 0, c_white, 1);
		
		//the arrow
		if option_pos == op
			{
			draw_sprite(spr_textbox_arrow, 0, _txtb_x, _txtb_y - _op_space*option_number + _op_space*op - 8);
			}
		
		//the option text
		draw_text(_txtb_x + 16 + _op_bord, _txtb_y - _op_space*option_number + _op_space*op + 2 - 8, option[op]);
		}
