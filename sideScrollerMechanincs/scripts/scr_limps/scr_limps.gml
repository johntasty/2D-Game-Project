...
function set_limps_moving (_armspeed, _shoulder, _hip,_legspeed )
{
	...
	var _movearm = _armspeed;
		...
		l_shoulder_x = x-lengthdir_x(_shoulderoffset,look_direction);
...
function set_limps_stop (_shoulder,_hip,_armlerp,_leglerp)
{
	...
	//arm shoot stationary
	var attack_frames = 0;
	if(global.__shtpress){
		var attack_frames = 10;
	}
	while (attack_frames > 0){
		var _lerpspdpunch = 0.2;
		//right arm
		r_elbow_x = lerp(r_elbow_x,r_shoulder_x + (lengthdir_x(bicep_length,look_direction_arm)),_lerpspdpunch);
		r_elbow_y = lerp(r_elbow_y, r_shoulder_y+(lengthdir_y( bicep_length,look_direction_arm)),_lerpspdpunch);
		r_elbow_y = clamp(r_elbow_y, (y-displacement)-bicep_length,(y-displacement)+bicep_length);
		//right hand
		r_hand_x = lerp(r_hand_x, r_elbow_x + lengthdir_x(bicep_length*2, look_direction_arm),_lerpspdpunch);
		r_hand_y = lerp(r_hand_y, r_elbow_y + (lengthdir_y( bicep_length*2, look_direction_arm)),_lerpspdpunch);
		...
		//part_type_direction(global.__sword_part,r_elbow_x,r_hand_x,0,0);
		part_type_orientation(global.__shine, p_dir, p_dir, 1, 0, 0);
		part_particles_create(global.__sword_part,r_hand_x,r_hand_y,global.__shine,2)
	...
	var _shoulderoffset = _shoulder;
	r_shoulder_x = x+lengthdir_x(_shoulderoffset,look_direction + 90);
	r_shoulder_y = (y-displacement)+lengthdir_y(_shoulderoffset,look_direction + 90);
	
	l_shoulder_x = x-lengthdir_x(_shoulderoffset,look_direction + 90);
	l_shoulder_y = (y-displacement)-lengthdir_y(_shoulderoffset,look_direction + 90);
