...
void main()
{
	...
	float ypart = fragCoord.y /fragCoord.y ;
	//
	float clip = 210.0;
	//i change this
	float ypartClip = 2.47 - (fragCoord.y/clip);
	float ypartClippedFalloff = clamp(2.0-ypartClip,0.0,1.0);
	float ypartClipped = min(ypartClip,1.0);
	float ypartClippedn = 1.0-ypartClipped;
	//
	float xfuel = 1.2-abs(2.0*xpart-1.0);//pow(1.0-abs(2.0*xpart-1.0),0.5);
	//
	float timeSpeed = 0.5;
	...
	vec3 mcolor = max(fire,sparks) ;
	float alpha = max(max(mcolor.r, mcolor.g), mcolor.b);
	vec4 color_new = vec4(mcolor.r, mcolor.g, mcolor.b, alpha) ;
	if(iFireColor == 2.){
		color_new = vec4(mcolor.b, mcolor.g, mcolor.r, alpha) ;
	}else if(iFireColor == 1.)
		color_new = vec4(mcolor.g, mcolor.r, mcolor.b, alpha) ;
	else{
		color_new = vec4(mcolor.r, mcolor.g, mcolor.b, alpha) ;
	}
	
	
	
	gl_FragColor = vec4(color_new);
